...
class _ManageShopScreenState extends State<ManageShopScreen>
  ...
  /// Shows a simple Yes/No skip confirmation dialog, then calls the API.
  void _showSkipDialog(String entryId, String customerName) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.person_off_rounded, color: AppColors.error, size: 22),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                await QueueService.instance.skipCustomer(entryId);
                if (mounted) {
                  _loadQueue();
