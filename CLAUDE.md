...
`app/routers/payments.py` implements Razorpay Standard Checkout for the two owner-side paid flows — subscriptions and Featured Promotions. Flow: `POST /payments/{subscription|promotion}/shop/{id}/create-order` creates a Razorpay order and logs a `payment_transactions` row (`status=created`); the Flutter app opens the Razorpay Checkout modal via `PaymentService` (`razorpay_flutter`); on success the 3 returned values (`razorpay_order_id`, `razorpay_payment_id`, `razorpay_signature`) go to `POST /payments/{subscription|promotion}/shop/{id}/verify`, which recomputes the HMAC-SHA256 signature (`app/services/razorpay_service.py`), finalizes the transaction row (`paid`/`failed`), and — only on a valid signature — calls the existing `subscription_service`/`promotion_service` to actually activate the plan/promotion. **`TEST_AMOUNT_PAISE = 100` (Rs. 1) is hardcoded for every order regardless of the selected plan/duration** — swap for real plan/promotion pricing before production. Requires `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` in `.env`; `key_id` (public) is returned to the client in the create-order response, `KEY_SECRET` never leaves the backend.
